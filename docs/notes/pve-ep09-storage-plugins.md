...
2026-09-15：承接第八集的卷激活步骤，按用户优先 PVE specific 的方向推进 D1 存储插件与卷（难度 8 / 优先级 10）。先交付 30 秒样片，获认可后已扩展为 [7 分钟完整课](storage-plugins-7min.md)；延后 G1 NUMA 的决定保持不变。
...
- 教学与动效：30 秒样片不承载后端能力表。完整课比较 NFS 文件路径、LVM-thin 块设备、RBD 用户态描述与 krbd，另外展开配置共享与数据共享；ZFS 内部机制留给独立主题。
- 故障设计：完整课分别增加 storage disabled、挂载失败、卷不存在；每个案例使用独立初态，不沿用正常路径的绿色结果。
