...
第四集写入持久性：[7 分钟完整课件](docs/notes/write-durability-7min.md)（`?sample=durability-lesson`）、[讲稿与源码依据](docs/notes/pve-ep04-write-durability.md)。`npm.cmd run render:durability-lesson` 导出 `out/write-durability-7min-v1.mp4`，展示 WRITE / FLUSH、两级易失缓存、完成返回及模式/FUA 对照。原 [30 秒机制样片](docs/notes/write-durability-30s.md)（`?sample=durability`）与断电预设继续保留。
