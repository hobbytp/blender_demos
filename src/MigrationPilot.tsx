...
export function MigrationPilot({failed=false}:{failed?:boolean}){
  ...
  const [handle]=useState(()=>delayRender('Migration pilot font'));
  ...
  return <AbsoluteFill style={{background:paper,fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label='热迁移内部机制：内存脏页、最终同步与执行交接'>
      ...
      <Text x={60} y={112} size={43} bold>{s.cancelled?'切换前失败，源端继续运行':titles[s.chapter]}</Text>
      <Text x={62} y={158} size={23} color={muted}>{s.cancelled?'限定分支：源端健康、尚未暂停交接；不能外推为任意失败都可回退':notes[s.chapter]}</Text>
      ...
      <Text x={1535} y={251} size={22} bold color={s.targetExecuting?green:muted}>{s.targetExecuting?'执行中':s.cleaned?'接收进程已清理':'仅接收 / 不执行'}</Text>
      ...
      {[s.source,s.destination].map((versions,side)=><g key={side} opacity={side===0&&!s.sourceProcessExists||side===1&&s.cleaned?.35:1}>
        {versions.map((version,i)=>{
          ...
          const flash=side===0&&((i===2&&f>=beats.dirty&&f<beats.dirty+18)||(i===5&&f>=beats.dirty+15&&f<beats.dirty+33)||(i===2&&f>=beats.redirty&&f<beats.redirty+18));
      ...
      {s.paused&&!s.deviceReady&&f>=beats.device&&<Wire points={[[870,365],[975,365],[975,701],[1080,701]]} frame={f} active color={amber} progress={ramp(f,beats.device,42)}/>}
      ...
      <Text x={1210} y={880} size={22} bold color={s.targetExecuting?green:amber}>{s.cancelled?'源端继续，目标不执行':s.targetExecuting?'目标继续原来的运行状态':s.paused?'暂停窗口 · 为讲解放大':'目标已有进程，但来宾未执行'}</Text>
      ...
      <Text x={1770} y={986} size={18} color={green} center>{s.cancelled?'失败对照':`${s.chapter+1} / 4`}</Text>
      <rect y={1010} width={1920} height={70} fill='#ecefe8'/><Text x={960} y={1055} size={30} center bold>{caption}</Text>
      <rect y={1075} width={1920*(f+1)/MIGRATION_DURATION} height={5} fill={green}/>
    </svg>
