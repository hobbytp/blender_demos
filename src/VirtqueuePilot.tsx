...
import {VIRTQUEUE_DURATION,virtqueueBeats as b,virtqueueStateAt} from './virtqueue-timeline';
...
export function VirtqueuePilot({poll=false}:{poll?:boolean}){
  ...
  const caption=poll?'轮询对照：不依赖完成中断；读取 used 后才能回收。':audio.find(c=>f>=c.start*30&&f<(c.start+c.duration+.3)*30)?.captions.filter(c=>c.start*30<=f).at(-1)?.text??'';
  ...
  return <AbsoluteFill style={{background:paper,fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label='Virtqueue 内部机制：发布、取用、完成与回收'>
      ...
      <Text x={60} y={110} size={42} bold>{poll?'轮询对照：完成依据仍然是 used 记录':['先写描述符，再发布可用环索引','后端按描述符访问数据，提交 TAP','先发布 used，再通知驱动回收','通知不是数据，回收不是远端收包'][s.chapter]}</Text>
      ...
      <Text x={1800} y={995} size={18} center color={green}>{poll?'轮询对照':`${s.chapter+1} / 4`}</Text>
      <rect y={1010} width={1920} height={70} fill='#ecefe8'/><Text x={960} y={1055} size={29} center bold>{caption}</Text>
      <rect y={1075} width={1920*(f+1)/VIRTQUEUE_DURATION} height={5} fill={green}/>
    </svg>
    {!poll&&audio.map(c=><Sequence key={c.file} from={Math.round(c.start*30)} durationInFrames={Math.round((c.end-c.start)*30)} layout='none'><Html5Audio src={staticFile(c.file)}/></Sequence>)}
  </AbsoluteFill>;
}
