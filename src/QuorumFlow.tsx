...
export function QuorumFlow() {
  const f = useCurrentFrame(), state = flowStateAt(f);
  ...
  const stream = (x1:number,x2:number,y:number,broken:boolean,breakAt=(x1+x2)/2) => <g>
    ...
    {[0,1].map(i=>{
      const p=((f/66+i/2)%1), mid=breakAt;
  ...
  return <AbsoluteFill style={{background:'#f8f7f2',fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label={titles[state.chapter]}>
      ...
      <Label x={1850} y={49} size={20} anchor='end' color='#74838a'>0{state.chapter+1} / 05</Label>
      <Label x={70} y={116} size={46} weight={750}>{titles[state.chapter]}</Label>
      <Label x={72} y={161} size={25} color='#667a81'>{notes[state.chapter]}</Label>
      ...
      {xs.map((x,i)=>{
        ...
        const internal=(start:number)=>ease(f,start,22);
        ...
        return <g key={x}>
          ...
          {i!==1 && <g opacity={ease(f,666,18)}>
            ...
            <Label x={x+240} y={927} size={22} anchor='middle' weight={600}>{f>=returnEnd?(minority?'× 写入被拒绝':'✓ 配置已保存'):'写入 VM 100 配置'}</Label>
      ...
      <rect y={1075} width={1920*(f+1)/900} height={5} fill={green}/>
    </svg>
