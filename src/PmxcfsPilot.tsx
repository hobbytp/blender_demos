...
import {PMXCFS_DURATION,pmxcfsBeats,pmxcfsStateAt,type PmxcfsTiming} from './pmxcfs-timeline';
...
export function PmxcfsPilot({blocked=false}:{blocked?:boolean}){
  ...
  return <><PmxcfsScene frame={f} state={s} blocked={blocked} title={blocked?titles[3]:titles[s.chapter]} caption={caption}/>{!blocked&&audio.map(c=><Sequence key={c.file} from={Math.round(c.start*30)} durationInFrames={Math.round((c.end-c.start)*30)} layout='none'><Html5Audio src={staticFile(c.file)}/></Sequence>)}</>;
}

export function PmxcfsScene({frame:f,state:s,blocked=false,title,caption,timing:b=pmxcfsBeats,duration=PMXCFS_DURATION,chapterCount=4,dim=false,children,note,shortMotion=false,objectLabel='同一对象：/etc/pve/nodes/B/qemu-server/100.conf · 描述字段示意'}:{frame:number;state:ReturnType<typeof pmxcfsStateAt>;blocked?:boolean;title:string;caption:string;timing?:PmxcfsTiming;duration?:number;chapterCount?:number;dim?:boolean;children?:ReactNode;note?:string;shortMotion?:boolean;objectLabel?:string}){
  ...
  const moving=(start:number,end:number)=>f>=start&&f<(shortMotion?Math.min(end,start+48):end);
  return <AbsoluteFill style={{background:paper,fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label='pmxcfs 内部机制：操作交付、本地副本与写入返回'>
      ...
      <Text x={60} y={108} size={42} bold>{title}</Text>
      <Text x={60} y={152} size={22} color={muted}>{note??(s.isolated?'分区已经稳定：A+B 有 quorum，C 无 quorum；不模拟本段之前的故障检测':'三节点已同步、无并发冲突；只跟踪一个已进入 FUSE 的配置写操作')}</Text>
      <Text x={60} y={192} size={23} bold color={blue}>{objectLabel}</Text>
      <g opacity={dim?.18:1}>
      {['A','B','C'].map((name,i)=>{
        const x=60+i*620,deniedNode=i===2&&s.isolated;
        const arrival=b.deliver+30+i*12,memAt=b.memory+[24,45,75][i],dbAt=[b.database+30,b.database+55,b.remoteDatabase??b.returned+45][i];
        return <g key={name}>
          ...
          <Wire points={[[x+155,355],[x+155,411]]} frame={f} active={i===0&&s.request&&!s.accepted&&moving(b.request,b.gate+20)||deniedNode&&f>=s.secondRequest&&!s.denied} color={deniedNode?red:blue}/>
          <Wire points={[[x+260,460],[x+295,460]]} frame={f} active={i===0&&s.accepted&&!s.sent&&moving(b.gate+20,b.send+35)} color={green} dashed={deniedNode}/>
          ...
          <Wire points={[[x+400,501],[x+400,532]]} frame={f} active={s.delivered[i]&&!s.memory[i]&&(!shortMotion||moving(memAt-Math.min(48,memAt-arrival),memAt))} progress={progress(f,shortMotion?memAt-Math.min(48,memAt-arrival):arrival,shortMotion?Math.min(48,memAt-arrival):memAt-arrival)} color={blue}/>
          ...
          <Wire points={[[x+280,622],[x+280,652]]} frame={f} active={s.memory[i]&&!s.database[i]&&(!shortMotion||moving(dbAt-Math.min(48,dbAt-memAt),dbAt))} progress={progress(f,shortMotion?dbAt-Math.min(48,dbAt-memAt):memAt,shortMotion?Math.min(48,dbAt-memAt):dbAt-memAt)} color={blue}/>
          ...
          {i===0&&<Wire points={[[x+50,696],[x+8,696],[x+8,322],[x+50,322]]} frame={f} active={s.localResult&&!s.returned&&(!shortMotion||moving(Math.max(b.localResult,b.returned-48),b.returned))} progress={progress(f,shortMotion?Math.max(b.localResult,b.returned-48):b.localResult,shortMotion?Math.min(48,b.returned-b.localResult):b.returned-b.localResult)} color={green}/>}
      ...
      <Wire points={[[570,880],[570,930],[1580,930],[1580,880]]} frame={f} active={s.sent&&!s.delivered[2]&&moving(b.send+35,b.deliver+54)} progress={progress(f,b.send+35,shortMotion?48:b.deliver+24-b.send-35)} color={blue} dashed={s.isolated}/>
      <Wire points={[[570,880],[570,930],[960,930],[960,880]]} frame={f} active={s.sent&&!s.delivered[1]&&moving(b.send+35,b.deliver+42)} color={blue}/>
      <Wire points={[[330,790],[330,766],[570,766],[570,790]]} frame={f} active={s.sent&&!s.delivered[0]&&moving(b.send+35,b.deliver+30)} color={blue}/>
      ...
      <Text x={1800} y={993} size={18} center color={green}>{blocked?'无 quorum':`${s.chapter+1} / ${chapterCount}`}</Text>
