...
import {BACKUP_DURATION,backupBeats,backupStateAt} from './backup-timeline';
const green='#347d70',blue='#347aa8',amber='#b18635',purple='#7e759c',muted='#73847e',paper='#f8f7f2';
export function BackupScene({frame:f,state:s,timing:b=backupBeats,duration=BACKUP_DURATION,title,caption='',dim=false,note,footer,scope,children}:{frame:number;state:ReturnType<typeof backupStateAt>&{sourceY?:string};timing?:typeof backupBeats&{startAck?:number};duration?:number;title?:string;caption?:string;dim?:boolean;note?:string;footer?:string;scope?:string;children?:ReactNode}){
  ...
  return <AbsoluteFill style={{background:paper,fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label='备份内部机制：冻结、保护、解冻与旧数据复制'>
      ...
      <Text x={60} y={110} size={42} bold>{title??['冻结的是文件系统，不是整台 VM','建立备份保护后解冻，复制继续','新写入遇到旧数据：先保护，再覆盖','文件系统、备份与应用：三个不同的状态'][s.chapter]}</Text>
      <Text x={60} y={158} size={23} color={muted}>{scope??'Linux VM · snapshot 备份模式 · QGA 正常 · 单虚拟磁盘 → VMA 归档 · 无 fleecing / 错误'}</Text>
      <Text x={60} y={202} size={22} bold color={blue}>{note??'块 X 尚未备份时遇到新写入：保护备份时间点的 X=v1；其他块仍待复制'}</Text>
      <g opacity={dim?.13:1}>
      ...
      <Card x={1150} y={645} w={320} title='运行中的虚拟磁盘' value={`X: ${s.sourceX} · Y: ${s.sourceY??'v1'}`} color={s.overwritten?purple:blue}/>
      ...
      {flow([[1680,355],[1680,310],[1115,310],[1115,455],[285,455],[285,565]],b.startAck??b.started+18,green,18)}
      ...
      {children}
      <Text x={60} y={duration===BACKUP_DURATION?952:965} size={22} bold color={amber}>{footer??`FS 状态：${s.frozen?'冻结':s.thawed?'已解冻':'运行 / 准备冻结'}　｜　备份：${s.started?'进行中':'待启动'}　｜　应用协调：未验证`}</Text>
      <Text x={60} y={duration===BACKUP_DURATION?990:997} size={18} color={muted}>琥珀：控制命令 · 绿：响应 / 已保护 · 紫：新写入 · 蓝：旧数据复制 · 无应用 hook / VSS · 时间非实测</Text>
      <Text x={1800} y={990} size={18} center color={green}>{s.chapter+1} / {duration===BACKUP_DURATION?4:8}</Text>
...
export function BackupPilot(){
  ...
  return <><BackupScene frame={f} state={s} caption={caption}/>
