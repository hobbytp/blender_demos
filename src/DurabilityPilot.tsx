...
import {DURABILITY_DURATION,durabilityBeats,durabilityStateAt} from './durability-timeline';
...
export function DurabilityPilot({powerLoss=false}:{powerLoss?:boolean}){
  ...
  return <DurabilityScene frame={f} state={s} powerLoss={powerLoss}/>;
}
export function DurabilityScene({frame:f,state:s,powerLoss=false,timing:b=durabilityBeats,duration=DURABILITY_DURATION,title,caption:customCaption,children,dim=false,shortMotion=false}:{frame:number;state:ReturnType<typeof durabilityStateAt>;powerLoss?:boolean;timing?:typeof durabilityBeats;duration?:number;title?:string;caption?:string;children?:ReactNode;dim?:boolean;shortMotion?:boolean}){
  ...
  const caption=customCaption??(powerLoss?(s.powerOff?'模拟断电：之前收到 WRITE OK，也不能保证新值留存。':'独立对照（无旁白）：选择回写尚未完成、未发出 FLUSH 的时刻断电。'):audio.find(c=>f>=c.start*30&&f<(c.start+c.duration+.3)*30)?.captions.filter(c=>c.start*30<=f).at(-1)?.text??'');
  const flow=(points:number[][],start:number,end:number,color=blue,enabled=true)=>{if(shortMotion)start=Math.max(start,end-36);return <Wire points={points} frame={f} active={enabled&&!s.powerOff&&f>=start&&f<end} progress={Math.max(0,Math.min(1,(f-start)/(end-start)))} color={color}/>;};
  return <AbsoluteFill style={{background:paper,fontFamily:'Quorum Noto, sans-serif'}}>
    <svg viewBox='0 0 1920 1080' width='100%' height='100%' role='img' aria-label='写入持久性：WRITE、FLUSH 与易失缓存的内部路径'>
      ...
      <Text x={60} y={110} size={42} bold>{title??(s.powerOff?'断电对照：WRITE 已完成，新值仍无保证':powerLoss?'对照：只完成 WRITE，随后模拟断电':titles[s.chapter])}</Text>
      ...
      <g opacity={s.powerOff?.3:1}>
        ...
        {flow([[1500,455],[1320,455]],b.stable,b.hostComplete,green,!powerLoss)}
        {flow([[960,450],[905,450],[905,675],[850,675]],b.hostComplete,b.backendComplete,green,!powerLoss)}
        {flow([[825,600],[825,475]],b.backendComplete,b.virtioComplete,green,!powerLoss)}
        {flow([[510,465],[450,465],[450,765],[390,765]],b.virtioComplete,b.flushAck,green,!powerLoss)}
      </g>
      ...
      <Text x={1800} y={995} size={18} center color={green}>{powerLoss?'断电对照':`${s.chapter+1} / ${duration===DURABILITY_DURATION?4:11}`}</Text>
    ...
    {duration===DURABILITY_DURATION&&!powerLoss&&audio.map(c=><Sequence key={c.file} from={Math.round(c.start*30)} durationInFrames={Math.round((c.end-c.start)*30)} layout='none'><Html5Audio src={staticFile(c.file)}/></Sequence>)}
  </AbsoluteFill>;
}
