...
import {COROSYNC_DURATION,corosyncChapters,corosyncStateAt} from './corosync-timeline';
const isCorosync = new URLSearchParams(location.search).get('sample') === 'corosync';
...
if(isCorosync) document.title='Corosync 成员重组 · PVE 图解课';
...
function App() {
  ...
  return <div className='page'>
    <header className='masthead'>
      ...
      <a className='edition' href={isCorosync ? '?sample=flow' : isStorage ? isStorageLesson ? '?sample=storage' : '?sample=storage-lesson' : isLifecycle ? isLifecycleLesson ? '?sample=lifecycle' : '?sample=lifecycle-lesson' : isRestore ? isRestoreLesson ? '?sample=restore' : '?sample=restore-lesson' : isBackup ? isBackupLesson ? '?sample=backup' : '?sample=backup-lesson' : isVirtqueue ? isVirtqueueLesson ? '?sample=virtqueue' : '?sample=virtqueue-lesson' : isDurability ? isDurabilityLesson ? '?sample=durability' : '?sample=durability-lesson' : isPmxcfs ? isPmxcfsLesson ? '?sample=pmxcfs' : '?sample=pmxcfs-lesson' : isMigrationContent ? isMigrationLesson ? '?sample=migration' : '?sample=migration-lesson' : isHaContent ? isHaLesson ? '?sample=ha' : '?sample=ha-lesson' : isLesson ? '?sample=flow' : '?sample=lesson'}>{isCorosync ? '对比：原 quorum 样片 ↗' : isStorage ? isStorageLesson ? '对比：30 秒存储样片 ↗' : '观看：7 分钟存储完整课 ↗' : isLifecycle ? isLifecycleLesson ? '对比：30 秒启动样片 ↗' : '观看：7 分钟启动完整课 ↗' : isRestore ? isRestoreLesson ? '对比：30 秒灾难恢复样片 ↗' : '观看：7 分钟灾难恢复完整课 ↗' : isBackup ? isBackupLesson ? '对比：30 秒备份机制样片 ↗' : '观看：7 分钟备份一致性完整讲解 ↗' : isVirtqueue ? isVirtqueueLesson ? '对比：30 秒 Virtqueue 样片 ↗' : '观看：7 分钟 Virtqueue 完整讲解 ↗' : isDurability ? isDurabilityLesson ? '对比：30 秒写入持久性样片 ↗' : '观看：7 分钟写入持久性完整讲解 ↗' : isPmxcfs ? isPmxcfsLesson ? '对比：30 秒 pmxcfs 机制样片 ↗' : '观看：7 分钟 pmxcfs 完整讲解 ↗' : isMigrationContent ? isMigrationLesson ? '对比：30 秒热迁移样片 ↗' : '观看：7 分钟热迁移完整讲解 ↗' : isHaContent ? isHaLesson ? '对比：30 秒机制样片 ↗' : '观看：7 分钟 HA 完整讲解 ↗' : isLesson ? '对比：30 秒动态样片 ↗' : '观看：75 秒完整试讲 ↗'}</a>
    </header>
    <main>
      <div className='intro'><div><p className='eyebrow'>{isCorosync?'从链路故障，追到新成员视图':isStorage?'从卷引用，追到实际数据路径':isLifecycle?'从一次启动，理解 PVE 管理链路':isRestore?'从归档恢复，走到业务证据':isBackup?'备份还在复制，为什么可以解冻？':isVirtqueue?'一份缓冲区，如何跨越 Guest / Host？':isDurability?'从一次写完成，追问数据保存在哪里':isPmxcfs?'从一次写入，理解配置复制':isMigrationContent?'从运行中的内存变化，理解热迁移':'从一次网络分区，理解集群协作'}</p><h1>{isCorosync ? 'Corosync 何时真正移除失联节点？' : isStorage ? 'PVE 如何把存储配置变成可用磁盘？' : isLifecycle ? '点击启动 VM 后，PVE 做了什么？' : isRestore ? '恢复完成之后，业务真的可用了吗？' : isBackup ? '备份与应用一致性：冻结保护了什么？' : isVirtqueue ? 'Virtqueue：通知、数据与完成如何协作？' : isDurability ? '写入返回成功，数据就能抗断电吗？' : isPmxcfs ? '一次配置修改，如何成为共享状态？' : isMigrationContent ? 'VM 还在写内存，如何完成切换？' : isHaContent ? '旧 VM 还活着，何时才敢接管？' : isDark ? '通信断了，谁还能写？' : '谁还能修改集群配置？'}</h1></div><p className='duration'>{isStorageLesson||isLifecycleLesson||isRestoreLesson||isBackupLesson||isVirtqueueLesson||isDurabilityLesson||isHaLesson||isMigrationLesson||isPmxcfsLesson?'7 分钟':`${DURATION / FPS} 秒`}<span>中文讲解 · 可暂停回看</span></p></div>
