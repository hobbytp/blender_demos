...
const DURATION = isBackupLesson ? BACKUP_LESSON_DURATION : isBackup ? BACKUP_DURATION : isVirtqueueLesson ? VIRTQUEUE_LESSON_DURATION : isVirtqueue ? VIRTQUEUE_DURATION : isDurabilityLesson ? DURABILITY_LESSON_DURATION : isDurability ? DURABILITY_DURATION : isPmxcfsLesson ? PMXCFS_LESSON_DURATION : isPmxcfs ? PMXCFS_DURATION : isMigrationLesson ? MIGRATION_LESSON_DURATION : isMigration ? MIGRATION_DURATION : isHaLesson ? HA_LESSON_DURATION : isHa ? HA_DURATION : isLesson ? LESSON_DURATION : isFlow ? FLOW_DURATION : isEditorial ? EDITORIAL_DURATION : isDark ? DARK_DURATION : originalDuration;
const chapters = isBackupLesson ? backupLessonChapters : isBackup ? backupChapters : isVirtqueueLesson ? virtqueueLessonChapters : isVirtqueue ? virtqueueChapters : isDurabilityLesson ? durabilityLessonChapters : isDurability ? durabilityChapters : isPmxcfsLesson ? pmxcfsLessonChapters : isPmxcfs ? pmxcfsChapters : isMigrationLesson ? migrationLessonChapters : isMigration ? migrationChapters : isHaLesson ? haLessonChapters : isHa ? haChapters : isLesson ? lessonChapters : isFlow ? flowChapters : isEditorial ? editorialChapters : isDark ? darkChapters : originalChapters;
...
function App() {
  ...
  return <div className='page'>
    <header className='masthead'>
      ...
      <a className='edition' href={isBackup ? isBackupLesson ? '?sample=backup' : '?sample=backup-lesson' : isVirtqueue ? isVirtqueueLesson ? '?sample=virtqueue' : '?sample=virtqueue-lesson' : isDurability ? isDurabilityLesson ? '?sample=durability' : '?sample=durability-lesson' : isPmxcfs ? isPmxcfsLesson ? '?sample=pmxcfs' : '?sample=pmxcfs-lesson' : isMigrationContent ? isMigrationLesson ? '?sample=migration' : '?sample=migration-lesson' : isHaContent ? isHaLesson ? '?sample=ha' : '?sample=ha-lesson' : isLesson ? '?sample=flow' : '?sample=lesson'}>{isBackup ? isBackupLesson ? '对比：30 秒备份机制样片 ↗' : '观看：7 分钟备份一致性完整讲解 ↗' : isVirtqueue ? isVirtqueueLesson ? '对比：30 秒 Virtqueue 样片 ↗' : '观看：7 分钟 Virtqueue 完整讲解 ↗' : isDurability ? isDurabilityLesson ? '对比：30 秒写入持久性样片 ↗' : '观看：7 分钟写入持久性完整讲解 ↗' : isPmxcfs ? isPmxcfsLesson ? '对比：30 秒 pmxcfs 机制样片 ↗' : '观看：7 分钟 pmxcfs 完整讲解 ↗' : isMigrationContent ? isMigrationLesson ? '对比：30 秒热迁移样片 ↗' : '观看：7 分钟热迁移完整讲解 ↗' : isHaContent ? isHaLesson ? '对比：30 秒机制样片 ↗' : '观看：7 分钟 HA 完整讲解 ↗' : isLesson ? '对比：30 秒动态样片 ↗' : '观看：75 秒完整试讲 ↗'}</a>
    </header>
    <main>
      <div className='intro'><div><p className='eyebrow'>{isBackup?'备份还在复制，为什么可以解冻？':isVirtqueue?'一份缓冲区，如何跨越 Guest / Host？':isDurability?'从一次写完成，追问数据保存在哪里':isPmxcfs?'从一次写入，理解配置复制':isMigrationContent?'从运行中的内存变化，理解热迁移':'从一次网络分区，理解集群协作'}</p><h1>{isBackup ? '备份与应用一致性：冻结保护了什么？' : isVirtqueue ? 'Virtqueue：通知、数据与完成如何协作？' : isDurability ? '写入返回成功，数据就能抗断电吗？' : isPmxcfs ? '一次配置修改，如何成为共享状态？' : isMigrationContent ? 'VM 还在写内存，如何完成切换？' : isHaContent ? '旧 VM 还活着，何时才敢接管？' : isDark ? '通信断了，谁还能写？' : '谁还能修改集群配置？'}</h1></div><p className='duration'>{isBackupLesson||isVirtqueueLesson||isDurabilityLesson||isHaLesson||isMigrationLesson||isPmxcfsLesson?'7 分钟':`${DURATION / FPS} 秒`}<span>中文讲解 · 可暂停回看</span></p></div>
