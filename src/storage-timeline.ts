...
export function storageStateAt(frame:number){const f=Math.max(0,Math.min(899,Math.floor(frame)));return {frame:f,chapter:storageChapters.findIndex(c=>f<c.end*30),configured:f>=lookup,plugin:f>=plugin,mounted:f>=mounted,checked:f>=checked,path:f>=path,opened:f>=opened,readRequested:f>=read,readCompleted:f>=completed,configurationCopiesData:false,pluginCarriesDiskIO:false,realExperiment:false};}
